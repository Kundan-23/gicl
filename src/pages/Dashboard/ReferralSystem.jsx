...
const ReferralSystem = () => {
  ...
  const handleEncash = () => {
    if (dashboardState.referralPoints > 0) {
      alert(`Successfully sent request to encash ${dashboardState.referralPoints} points! Our team will contact you.`);
    } else {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 className="heading-1">Refer a Friend</h1>
        <p className="text-secondary" style={{ marginTop: '0.5rem' }}>Earn 0.5 points for every direct referral and network bonuses!</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div style={{ backgroundColor: 'var(--bg-surface)', padding: '2rem', borderRadius: 'var(--radius-xl)', border: '1px solid var(--brand-primary)', textAlign: 'center' }}>
          ...
          <p className="text-secondary" style={{ marginBottom: '1.5rem' }}>Share this code with your friends when they register.</p>
        ...
        <div style={{ backgroundColor: 'var(--bg-surface)', padding: '2rem', borderRadius: 'var(--radius-xl)', border: '1px solid var(--bg-surface-elevated)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            ...
            <span style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--brand-primary)' }}>{dashboardState.referralPoints}</span>
          </div>

          <p className="text-small" style={{ marginBottom: '1.5rem', opacity: 0.8 }}>
            Points can be encashed for real rewards. You earn 0.5 points when someone registers using your code, and bonus points when they refer others!
          </p>

          <button className="btn-secondary" style={{ marginTop: 'auto' }} onClick={handleEncash} disabled={dashboardState.referralPoints === 0}>
            Encash Points
          </button>
        </div>
      ...
      <div style={{ backgroundColor: 'var(--bg-surface)', padding: '1.5rem', borderRadius: 'var(--radius-xl)', border: '1px solid var(--bg-surface-elevated)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          {/* Dev button to test functionality */}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--bg-surface-elevated)', color: 'var(--text-secondary)' }}>
                <th style={{ padding: '1rem', fontWeight: 500 }}>Referred Player</th>
